...
```bash
eksctl create cluster --version=1.18 --name=eks-spot-lab --node-private-networking --managed --nodes=3 --alb-ingress-access --region=us-east-1 --node-type t4g.medium --node-labels="lifecycle=OnDemand" --asg-access
```
### kube-ops-view
